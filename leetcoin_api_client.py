...
class LeetCoinAPIClient():
    ...
    def __init__(self, url, api_key, shared_secret, encryption, debug=True):
        ...
        for i in range(0, max_threads):
            self.workers.append(Worker(i, self.shareddata, debug, encryption, self.allow_non_authorized_players))
...
#
    ...
    def get_https_response(self, params, uri, debug=False):
        if debug:
            print ("[1337] [nonthread] get_https_response %s" % (uri))
...
class Worker(threading.Thread):
    ...
    def get_https_response(self, params, uri):
